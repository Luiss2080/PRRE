import React, { useEffect, useState } from 'react';

/**
 * PantallaCarga
 * Componente que representa la pantalla de carga animada que se muestra
 * al ingresar al portal por primera vez, con animaciones de desvanecimiento (fade out).
 */
export default function PantallaCarga({ alTerminar }) {
  // Estado que controla la animación de desvanecimiento (fade out) de la pantalla de carga
  const [desvanecer, setDesvanecer] = useState(false);

  // Al montar la pantalla de carga, espera 1.8 segundos y luego activa la transición de fade out.
  // 500ms después, finaliza por completo y llama a la función de callback alTerminar.
  useEffect(() => {
    const temporizador = setTimeout(() => {
      setDesvanecer(true);
      const temporizadorFinal = setTimeout(() => {
        if (alTerminar) alTerminar();
      }, 500);
      return () => clearTimeout(temporizadorFinal);
    }, 1800);

    return () => clearTimeout(temporizador);
  }, [alTerminar]);

  return (
    <div style={estiloContenedor(desvanecer)}>
      <div style={estiloContenido}>
        {/* Contenedor del logotipo U.E. Germán Busch B con efecto de resplandor neón */}
        <div style={estiloEnvolturaLogo}>
          <img 
            src="/LogoInstitucional.png" 
            alt="Logo U.E. Germán Busch B" 
            style={estiloImagenLogo} 
          />
        </div>

        {/* Título de la aplicación con degradado metálico cian/naranja */}
        <h1 style={estiloTitulo}>PRRE</h1>
        <p style={estiloSubtitulo}>Portal de Reserva de Recursos Educativos</p>
        
        {/* Plaqueta institucional con el nombre del colegio */}
        <div style={estiloEtiquetaEscuela}>
          <span>U. E. GERMÁN BUSCH B</span>
        </div>

        {/* Barra de progreso de carga animada */}
        <div style={estiloContenedorCarga}>
          <div style={estiloBarraCarga}></div>
        </div>
      </div>
    </div>
  );
}

// Estilos en línea para la Pantalla de Carga (Modo Oscuro Fijo)
const estiloContenedor = (desvanecer) => ({
  position: 'fixed',
  top: 0,
  left: 0,
  right: 0,
  bottom: 0,
  backgroundColor: '#05080e', // Fondo negro obsidiana profundo
  color: '#ffffff',
  display: 'flex',
  flexDirection: 'column',
  alignItems: 'center',
  justifyContent: 'center',
  zIndex: 9999,
  opacity: desvanecer ? 0 : 1,
  visibility: desvanecer ? 'hidden' : 'visible',
  transition: 'opacity 0.5s ease, visibility 0.5s ease',
  textAlign: 'center',
  padding: '1.5rem',
});

const estiloContenido = {
  display: 'flex',
  flexDirection: 'column',
  alignItems: 'center',
  maxWidth: '450px',
  width: '100%',
};

const estiloEnvolturaLogo = {
  marginBottom: '1.5rem',
  filter: 'drop-shadow(0 0 25px rgba(0, 229, 255, 0.4))',
  animation: 'pulseLogo 2s infinite ease-in-out',
};

const estiloImagenLogo = {
  width: '140px',
  height: 'auto',
  display: 'block',
};

const estiloTitulo = {
  fontSize: '3.25rem',
  fontWeight: '900',
  letterSpacing: '0.12em',
  background: 'linear-gradient(135deg, #00e5ff 0%, #FF9F1C 100%)',
  WebkitBackgroundClip: 'text',
  WebkitTextFillColor: 'transparent',
  marginBottom: '0.25rem',
  filter: 'drop-shadow(0 4px 12px rgba(0, 229, 255, 0.15))'
};

const estiloSubtitulo = {
  fontSize: '0.875rem',
  color: '#94A3B8',
  fontWeight: '700',
  letterSpacing: '0.1em',
  marginBottom: '1.5rem',
  textTransform: 'uppercase',
};

const estiloEtiquetaEscuela = {
  backgroundColor: 'rgba(255, 159, 28, 0.15)',
  border: '1px solid rgba(255, 159, 28, 0.3)',
  padding: '0.45rem 1.25rem',
  borderRadius: '50px',
  color: '#FF9F1C',
  fontSize: '0.75rem',
  fontWeight: '800',
  letterSpacing: '0.12em',
  marginBottom: '3rem',
  boxShadow: '0 0 15px rgba(255, 159, 28, 0.1)'
};

const estiloContenedorCarga = {
  width: '180px',
  height: '4px',
  backgroundColor: 'rgba(255, 255, 255, 0.08)',
  borderRadius: '10px',
  overflow: 'hidden',
  position: 'relative',
};

const estiloBarraCarga = {
  width: '70%',
  height: '100%',
  backgroundColor: '#00e5ff',
  borderRadius: '10px',
  position: 'absolute',
  animation: 'loadingProgress 1.6s ease-in-out infinite',
  boxShadow: '0 0 10px #00e5ff',
};

// Inyección dinámica de animaciones CSS en la cabecera del documento
if (typeof document !== 'undefined') {
  const styleTag = document.createElement('style');
  styleTag.textContent = `
    @keyframes pulseLogo {
      0%, 100% { transform: scale(1); filter: drop-shadow(0 0 15px rgba(0, 229, 255, 0.3)); }
      50% { transform: scale(1.05); filter: drop-shadow(0 0 30px rgba(255, 159, 28, 0.45)); }
    }
    @keyframes loadingProgress {
      0% { left: -100%; width: 30%; }
      50% { width: 40%; }
      100% { left: 100%; width: 30%; }
    }
  `;
  document.head.appendChild(styleTag);
}
